...
public class TabIndicatorView extends RelativeLayout {
	...
	public void setTabIcon(String normalIconUrl, String focusIconUrl,boolean isLocalDrawable) {
       ...
       //默认是什么样子的 工具给其附图片
		if (isLocalDrawable) {
			...
			xUtilsImageUtils.displayIN(ivTabIcon,R.mipmap.ic_error,normalIconUrl);
		}
	...
	public void setTabSelected(boolean selected) {
		if (selected) {
			tvTabHint.setTextColor(context.getResources().getColor(R.color.mainred));
			if (isLocalDrawable) {
				ivTabIcon.setImageResource(Integer.parseInt(focusIconUrl));
			}else {
				xUtilsImageUtils.display(ivTabIcon,R.mipmap.ic_error,focusIconUrl);
			}
			
		
		} else {
			tvTabHint.setTextColor(context.getResources().getColor(R.color.tabunchoose));
			if (isLocalDrawable) {
				ivTabIcon.setImageResource(Integer.parseInt(normalIconUrl));
			}else {
				xUtilsImageUtils.display(ivTabIcon,R.mipmap.ic_error,normalIconUrl);
			}
