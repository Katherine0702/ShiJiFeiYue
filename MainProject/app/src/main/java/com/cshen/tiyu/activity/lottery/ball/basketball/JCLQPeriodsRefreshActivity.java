...
public class JCLQPeriodsRefreshActivity extends Activity implements  OnFooterRefreshListener {
	...
	private void initHead() {
		...
		backImageView.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				// TODO 自动生成的方法存根
				Intent intent = new Intent(JCLQPeriodsRefreshActivity.this,
						MainActivity.class);
				startActivity(intent);
	...
	private void initFirstData() {
		httpTask = true;
		start = more * count;
		ServiceCaiZhongInformation.getInstance().new_getZc_Macth_Result(_this,null,start,count,ConstantsBase.JCLQ+"",
	...
	private void initView() {
		((TextView)findViewById(R.id.tv_head_title)).setText("竞彩篮球");
		((TextView)findViewById(R.id.notice)).setText("彩果为48分钟内的比赛结果（含伤停补时）");
	...
	public void onFooterRefresh(PullToRefreshView view) {
		...
		if (httpTask == false) {
			httpTask = true;
			start = more * count;
			ServiceCaiZhongInformation.getInstance().new_getZc_Macth_Result(_this,null,start,
					count,ConstantsBase.JCLQ+"",new CallBack<HttpZcMatchListResult>() {
				@Override
				public void onFailure(ErrorMsg errorMessage) {
					httpTask = false;
					PostHttpInfoUtils.doPostFail(_this, errorMessage, "访问失败");
	...
	class JczqAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			ViewHolder holder;
			if (convertView == null) {
				convertView = View.inflate(_this, R.layout.jclq_period_item,
						null);
				...
				holder.tv_text_rangfen=(TextView) convertView.findViewById(R.id.tv_text_rangqiu);
				holder.tv_SF=(TextView) convertView.findViewById(R.id.tv_SF);
				holder.tv_RFSF=(TextView) convertView.findViewById(R.id.tv_RFSF);
				holder.tv_SFC=(TextView) convertView.findViewById(R.id.tv_SFC);
				holder.tv_DXF=(TextView) convertView.findViewById(R.id.tv_DXF);
			...
			if (data != null) {
				...
				if (data.getHandicap()!=null ) {
					holder.tv_text_rangfen.setText(Html.fromHtml("让分"+"<font color='#56a81f'>"+"("+data.getHandicap()+")"+"</font>"));
				}
				...
				if (!TextUtils.isEmpty(result) && !TextUtils.isEmpty(reslutSP)) {
					...
					holder.tv_bifen.setText(Html.fromHtml(
							"<font color='#FF3232'>"+data.getHomeScore()+":"+data.getGuestScore()+"</font>"));


					String SF= getHtmlStr(results[0],resultSPs[0]);
					String RFSF= getHtmlStr(results[1],resultSPs[1]);
					String SFC= getHtmlStr(results[2],resultSPs[2]);
					String DXF= getHtmlStr(results[3],resultSPs[3]);
					holder.tv_SF.setText(Html.fromHtml(SF));
					holder.tv_RFSF.setText(Html.fromHtml(RFSF));
					holder.tv_SFC.setText(Html.fromHtml(SFC));
					holder.tv_DXF.setText(Html.fromHtml(DXF));
				}else{
					return null;
				}
			}
		...
		class ViewHolder {
			...
			public  TextView tv_text_rangfen;
			public  TextView tv_SF;
			public  TextView tv_RFSF;
			public  TextView tv_SFC;
			public  TextView tv_DXF;
		}
