...
public class SSQMainActivity  extends BaseActivity  implements OnClickListener,SensorEventListener  {
	public static final int HAVENUMBER = 2;
	private SSQMainActivity _this;
	...
	private TextView qianqutext,houqutext;
	private ImageView clear;//清除
	private View sure;//确定
	private int[] reds33View = new int[]{//红球View、方便移动
			...
			R.id.dlt_numview31,R.id.dlt_numview32,R.id.dlt_numview33};
	private int[] blues16View = new int[]{//蓝球View、方便移动
			...
			R.id.dltnumview11,R.id.dltnumview12,R.id.dltnumview13,R.id.dltnumview14
			,R.id.dltnumview15,R.id.dltnumview16};
	private int[] reds33 = new int[]{//红球
			...
			R.id.dlt_num31,R.id.dlt_num32,R.id.dlt_num33};
	private int[] blues16 = new int[]{//蓝球
			...
			R.id.dltnum11,R.id.dltnum12,R.id.dltnum13,R.id.dltnum14,R.id.dltnum15,R.id.dltnum16};
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.ssqmain);
		_this = this;
		initView();
		initdata(); 
	}
	...
	public void initView(){
		...
		tv_head.setTopClickItemListener(new TopClickItemListener() {
			...
			public void clickLoginView(View view) {
				...
				intentHelp.putExtra("url","http://an.caiwa188.com/help/ssq.html");
		...
		qianqutext = (TextView) findViewById(R.id.qianqutext);
		qianqutext.setText("前区至少选择6个");
		houqutext = (TextView) findViewById(R.id.houqutext);
		houqutext.setText("后区至少选择1个");
		...
		for(int i=0;i<reds33.length;i++){
			findViewById(reds33[i]).setOnClickListener(this);
		}
		for(int i=0;i<blues16.length;i++){
			findViewById(blues16[i]).setOnClickListener(this);
		}
		...
		choose6.setBackgroundResource(R.drawable.dlt_number_back_red);
	...
	private void initdata() {
		...
		try{
			...
			if(ssqNumber!=null){
				qianqu = ssqNumber.getQianqu();
				houqu = ssqNumber.getHouqu();
				setNumbersChoosed(qianqu);
				setNumbersChoosed(houqu);
				String moneyStr = "<html><font color=\"#000000\">"+ssqNumber.getNum()+"注"
						+"</font><font color=\"#e73c42\">"+ssqNumber.getNum()*2+""
						+ "</font><font color=\"#000000\">元</font></html>";
				money.setText(Html.fromHtml(moneyStr));
			}	
		}catch(Exception e){
	...
	public void onClick(View v) {
			...
			if(!isMoving){
				if(qianqu == null&&houqu == null){//没选择过
					...
					if(qianqu != null&&houqu != null){//都选择了
						if(qianqu.getNumbers().size()<6){
							ToastUtils.showShort(_this, "前区至少选择6个");					
							return;
						}
						if(qianqu.getNumbers().size()>16){
							ToastUtils.showShort(_this, "前区至多选择16个");					
							return;
						}
						if(houqu.getNumbers().size()<1){
							ToastUtils.showShort(_this, "后区至少选择1个");					
							return;
						}
						if(ssqNumber == null){
							ssqNumber = new DLTNumber();
						}
						ssqNumber.setQianqu(ChooseUtil.getChooseUtil().getSortList(qianqu));
						ssqNumber.setHouqu(ChooseUtil.getChooseUtil().getSortList(houqu));
					}else{//选择了一个
						if(qianqu == null){
							ToastUtils.showShort(_this, "前区至少选择6个");	
						}
				...
				ssqNumber.setNum(ChooseUtil.getChooseUtil().getTimeSSQ(
						ssqNumber.getQianqu().getNumbers().size(),
						ssqNumber.getHouqu().getNumbers().size()));
				if(position == -1){
					Intent intent = new Intent(_this, SSQAccountListActivity.class);
					Bundle bundle = new Bundle();   
					bundle.putSerializable("ssqNumber", ssqNumber);
					...
					bundle.putSerializable("dltNumber", ssqNumber);
					intent.putExtras(bundle);
		...
		default:
			Arrays.sort(reds33);  
			Arrays.sort(blues16);  
			int indexRed = Arrays.binarySearch(reds33, v.getId()); 
			int indexBlue = Arrays.binarySearch(blues16, v.getId());  
			if(indexRed>=0){//排位从0开始，初始值从1开始
				...
				if(qianqu.getNumbers().contains(indexRed+1)){
					...
					seColorBack(reds33[indexRed],"#E73C42",R.drawable.dlt_number_back);
					...
					seColorBack(reds33[indexRed],"#FFFFFF",R.drawable.dlt_number_back_red);
				}
			}else if(indexBlue>=0){
				if(houqu == null){
				...
				if(houqu.getNumbers().contains(indexBlue+1)){
					...
					seColorBack(blues16[indexBlue],"#3389F1",R.drawable.dlt_number_back);
					if(houqu.getNumbers().size() == 0){
						houqu = null;
					}
				}else{
					houqu.getNumbers().add(indexBlue+1);
					seColorBack(blues16[indexBlue],"#FFFFFF",R.drawable.dlt_number_back_blue);
				}
			}			
			...
			if(qianquSize>=6&&houquSize>=1){
				int time = ChooseUtil.getChooseUtil().getTimeSSQ(
						qianquSize,
						houquSize);
	...
	public void clear(){
		...
		ssqNumber = null;
		for(int i=0;i<reds33.length;i++){
			seColorBack(reds33[i],"#E73C42",R.drawable.dlt_number_back);
		}
		for(int i=0;i<blues16.length;i++){
			seColorBack(blues16[i],"#3389F1",R.drawable.dlt_number_back);
		}
	}
	public void jiXuanChoose(){
		clear();
		ssqNumber =  ChooseUtil.getChooseUtil().getRandomSSQ();
		qianqu = ssqNumber.getQianqu();
		houqu = ssqNumber.getHouqu();
		setNumbersChoosed(qianqu);
		setNumbersChoosed(houqu);
	...
	public void setNumbersChoosed(NumberEach numbers){
		if(numbers.getType() == 0){//前区
			for(int i=0;i<numbers.getNumbers().size();i++){
				seColorBack(reds33[numbers.getNumbers().get(i)-1],"#FFFFFF",R.drawable.dlt_number_back_red);
			}
		}
		if(numbers.getType() == 1){//后区
			for(int j=0;j<numbers.getNumbers().size();j++){
				seColorBack(blues16[numbers.getNumbers().get(j)-1],"#FFFFFF",R.drawable.dlt_number_back_blue);
			}
		}
	}
	...
	private void getNetDate() {
		...
		ServiceCaiZhongInformation.getInstance().pastPrizeList(_this,ConstantsBase.SSQ+"","0","10","",new CallBack<PrizeList>() {
			@Override
				...
				if(t!=null&&t.getPeriodList()!=null
					...
					if(period!=null){
						...
						if(period.getEndJoinTime_single().length()>16){
							...
							PreferenceUtil.putString(_this,PreferenceUtil.SSQPRIZEPERIOD,periodTime);
	...
	public void setBigToSmallView(){
		...
		for(int i=0;i<houqu.getNumbers().size();i++){
			if(houqu.getNumbers().get(i)<10){
				choosees[6+i].setText("0"+houqu.getNumbers().get(i)+"");
			}else{
				choosees[6+i].setText(houqu.getNumbers().get(i)+"");
			}
		}
	}
	...
	public void setToPlace(){
		for(int i=0;i<choosees.length;i++){
			...
			if(i<6){
				view = findViewById(reds33View[qianqu.getNumbers().get(i)-1]);
			}else{
				view = findViewById(blues16View[houqu.getNumbers().get(i-6)-1]);
			}
			if(i<6){
				viewTextView = (TextView) findViewById(reds33[qianqu.getNumbers().get(i)-1]);
			}else{
				viewTextView = (TextView) findViewById(blues16[houqu.getNumbers().get(i-6)-1]);
			}	
			// TODO Auto-generated method stub
	...
	public TranslateAnimation toPlace(final int index,final View viewFrom,final TextView viewToTextView,float leftFrom,float topFrom,float leftTo,float topTo)
	{
